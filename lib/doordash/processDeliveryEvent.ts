...
export type DoorDashEvent = (typeof VALID_EVENTS)[number];
...
export type ProcessOutcome =
  ...
  | { status: 'unmatched'; deliveryId: string };
...
async function broadcastDeliveryUpdate(
  ...
  if (
    ...
    console.warn('Supabase not configured; skipping realtime delivery broadcast');
    return;
  }
...
/**
 * Apply one DoorDash delivery event to its order.
 *
 * Shared by the Next.js webhook route and the SQS consumer so the two paths
 * can't drift. Throws only on failures worth retrying — a database error, a
 * dropped connection. Anything the payload itself makes impossible (unknown
 * event, no matching order) returns rather than throwing, because retrying it
 * five times on the way to a dead-letter queue helps nobody.
 */
export async function processDeliveryEvent(
  ...
  if (!event_name || !VALID_EVENTS.includes(event_name)) {
    console.warn('DoorDash event: unknown event_name', event_name);
    return { status: 'ignored', reason: `unknown event_name: ${event_name}` };
  }

  const doordashDeliveryId = delivery_id || external_delivery_id;
  if (!doordashDeliveryId) {
    return { status: 'ignored', reason: 'no delivery identifier in payload' };
  }
  ...
  if (!order) {
    console.warn('DoorDash event: order not found for delivery', doordashDeliveryId);
    return { status: 'unmatched', deliveryId: doordashDeliveryId };
  }
  ...
  // Only stamp these once. DoorDash can resend an event, and SQS redelivers on
  // a visibility timeout — without the guard, a duplicate DASHER_PICKED_UP
  // without an explicit timestamp would overwrite the real pickup time with
  // whenever the retry happened to run.
  if (event_name === 'DASHER_PICKED_UP' && !order.actualPickupTime) {
    updates.actualPickupTime = body.actual_pickup_time
      ? new Date(body.actual_pickup_time)
      : new Date();
  }

  if (event_name === 'DASHER_DROPPED_OFF' && !order.actualDropoffTime) {
  ...
  // The broadcast is a nicety — the tracking page polls as well — so a failure
  // here must not cause the message to be retried and the order re-updated.
  try {
  ...
  console.log(`DoorDash event processed: ${event_name} for order ${order.id}`);
